...
public class ThreeWayQuicksort {
	...
	private static void qSorter(int[] a, int lo, int hi) {
		...
		/*
		 * If the sub array is of size 10 or less it is better to use insertion sort
		 * instead
		 */
		if (hi - lo <= 0) {
		...
		if (hi - lo <= 0) {
			/*
			 * If the sub array is small, we can just use the middle element as the element
			 * to compare with
			 */
			swap(a, lo, (lo + hi) / 2);
		} else if (hi - lo <= 0) {
			/*
			 * If the sub array is medium in size, we can use the median of three elements
			 * lo, (mid), and hi and set it as the element to compare with.
			 */
			...
			/*
			 * Debug code System.out.println("Median of "+Arrays.toString(a1));
			 * System.out.println("Median of "+Arrays.toString(a2));
			 * System.out.println("Median of "+Arrays.toString(a3));
			 */

			int median = median(a, median(a, a1[0], a1[1], a1[2]), median(a, a2[0], a2[1], a2[2]),
